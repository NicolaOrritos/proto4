...
function rangeModifier(termScore, correlationStrength)
{
    ...
    if (termScore)
    {
        ...
        if (typeof(correlationStrength) !== "undefined")
        {
        ...
        console.log("Range modifier is '%s * %s = %s'", ratio, termScore, modifier);
        
        result[0] = termScore - modifier;
        result[1] = result[0] + 2 * modifier; // termScore may be a string... damn
        
        // Correction for small sets of words:
        result[0]--;
        result[1]++;
    }
...
{
    console.log("Complete smembers called");
    ...
    function run()
    {
        ...
        if (callback)
        {
            callback.call(this, error, length, position, partialResult);
        }
    }
...
var Searcher = function()
{
    ...
    this.CORRELATION_STRONG = 1/100;
    this.CORRELATION_NORMAL = 1/50;
    this.CORRELATION_LOW = 1/10;
    this.CORRELATION_VERYLOW = 1;
    ...
    {
        this.correlatedWithinPageWithStrength(docID, pageID, term, this.CORRELATION_NORMAL, callback);
    };
    ...
    this.correlatedWithinPageWithStrength = function(docID, pageID, term, correlationStrength, callback)
    {
        ...
        if (typeof(term)   !== "undefined")
        {
            function run()
            {
                ...
                // console.log("termScore for '%s' is %s [searching with pattern '%s']", term, termScore, pattern);
    ...
    // [bug] - A complex object is returned as first element of the list: maybe sets.js is at fault here
                ...
                if (keys.length > 0)
                {
                    ...
                    for (var a=0; a<keys.length; a++)
                    {
                        ...
                        if (parts[0] == docID)
                        {
                            ...
                            correlated = wait.forMethod(_self, "correlatedWithinPageWithStrength", docID, parts[1], term, this.CORRELATION_EXACT);
