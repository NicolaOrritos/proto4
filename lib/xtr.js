...
var xtractor = function()
{
    ...
    this.allCorrelatedTo = function(term, callback)
    {
        if (term)
        {
            // 1. Get documents containing this term with its occurrency-ratio per document
            var args2 = [ term, "+inf", "-inf", 'WITHSCORES', 'LIMIT', 0, 1 ];
            redis.zrevrangebyscore(args2, function(err, result)
            {
                var docs = new Array();

                if (err)
                {
                    console.log("Could not get docs containing term '%s'. Cause: %s", term, err);
                }
                else
                {
                    for (var a=0; a<result.length; a++)
                    {
                        var parts = result[a].split(":");
