...
function pushTermTo(terms, term)
{
    if (term)
    if (terms)
    {
        // [todo] - Add multi-terms splitting to 'pushTermTo()'
        ...
        else
        {
            ...
            term = term.replace("ftp://", "");
            
            // console.log("Term after chores #6:  '%s'", term);


            if (term.length > 0)
            {
                // [todo] - Remove trailing ".", "_", "—" and "-"
                
                if (   term[term.length - 1] == '-'
                    || term[term.length - 1] == '—'
                    || term[term.length - 1] == '_'
                    || term[term.length - 1] == '.')
                {
                    term = term.substring(0, term.length - 1);
            
                    // console.log("Term after chores #7:  '%s'", term);
                }
                    ...
                    || term[0] == '.')
                {
                    ...
                    // console.log("Term after chores #8:  '%s'", term);
                }
                
                // console.log("Pushing term '%s'...", term);
                ...
                else
                {
                    ...
                    for (var i=0; i<pages.length; i++)
                    {
                        ...
                        if (text)
                        {
                            ...
                            {
                                pushTermTo(usefulTerms, terms[a]);
                                
                                // [todo] - Implement pushTermTo(term, usefulTerms) as a separate function
                            }
