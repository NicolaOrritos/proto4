        ...
        if (typeof(term)   !== 'undefined')
        {
            function run()
            {
                ...
                if (termScore)
                {
                    ...
                    var rangeModifier = 1/10 * termScore;
                    
                    var lowRange = termScore - rangeModifier;
                    var topRange = lowRange + 2 * rangeModifier; // termScore is a string... damn
                    
                    console.log("score range is: [%d, %d]", lowRange, topRange);

                    // 3. Once got the desired score-range, ask for terms that stay within it
                    result = wait.forMethod(redis, "zrevrangebyscore", pattern, topRange, lowRange);
                }
