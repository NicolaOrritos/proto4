...
var REDIS_DB = 12;
var DOCS_KEY = "docs";
var DOC_WIDE_INFO_PREFIX = "docwide";
var DOC_PAGE_INFO_PREFIX = "docpage";
...
function pushTerms(docID, pageID, terms, length2, position2, callback)
{
    ...
    if (terms)
    {
        ...
        for (var i=0; i<termsList.length; i++)
        {
            ...
            sadd_async(DOC_PAGE_INFO_PREFIX + ":" + term,
                       key,
                       termsList.length,
                       i + 1,
...
var xtractor = function()
{
                    ...
                    function metacallback(length, position, result)
                    {
                        ...
                        if (length == position)
                        {
                            ...
                            for (var c=0; c<termsWithDocScores.length; c++)
                            {
                                ...
                                if (termWithDocScore)
                                {
                                    zadd_async(DOC_WIDE_INFO_PREFIX + ":" + termWithDocScore.term,
                                               termWithDocScore.docScore,
                                               doc);     // term => doc(doc-wide-score)
                                }
                            }
                        }
                    }
    ...
    this.search = function(term, callback)
    {
        if (term)
        {
            function run()
            {
                term = DOC_PAGE_INFO_PREFIX + ":" + term;
